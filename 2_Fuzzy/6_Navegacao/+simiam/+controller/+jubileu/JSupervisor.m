...
% see the LICENSE file included with this software
    ...
    %% METHODS
        ...
        %% SUPERVISOR Constructor
            ...
            obj.controllers{1} = simiam.controller.Fuzzy_Stop();
            obj.controllers{2} = simiam.controller.FuzzySeguirAngulo();
            obj.controllers{3} = simiam.controller.FuzzyIrParaMeta();
            obj.controllers{4} = simiam.controller.FuzzyEvitarObstaculo();
            obj.controllers{5} = simiam.controller.Fuzzy_EO_IPO();
            obj.controllers{6} = simiam.controller.FuzzySeguirParede();
            obj.controllers{7} = simiam.controller.FuzzyIPO_EO_SP();

            % set the initial controller
            obj.current_controller = obj.controllers{7};
            obj.current_state = 7;
        ...
        %   See also controller/execute
            ...
            if obj.is_in_state('fuzzy_stop') && ~obj.check_event('at_goal')
                obj.switch_to_state('ControladorFuzzy');
            elseif(obj.check_event('at_goal'))
                obj.switch_to_state('fuzzy_stop');
            end
...
%                 fprintf('already in state %s\n', name);
        ...
        %   measured wheel encoder tick counts are stored in prev_ticks.
            ...
            % Compute odometry here - constantes uteis
            R = obj.robot.wheel_radius;
            L = obj.robot.wheel_base_length;
            m_per_tick = (2*pi*R)/obj.robot.encoders(1).ticks_per_rev;
            
            % Variação em x, y, theta
            dl = left_ticks - prev_left_ticks;
            dr = right_ticks - prev_right_ticks;
            Dc = m_per_tick*(dl + dr)/2;
            
            x_dt = Dc*cos(theta);
            y_dt = Dc*sin(theta);
            theta_dt = m_per_tick*(dr-dl)/L;
            
            % Add deltas on current pose
