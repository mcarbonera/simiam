%% PARA VERIFICAR: valor beta para pwm e linha 390 (limites para valor bruto)
...
%            obj.add_surface_with_alpha(qb_axle, [0.15 0.15 0.15], 0.5);
            ...
            obj.ir_array(1) = ProximitySensor(parent, 'IR', pose, ir_pose, 0.1, 0.8, Pose2D.deg2rad(6), 'simiam.robot.Jubileu.ir_distance_to_raw');
            
            ir_pose = Pose2D(0.0613+offsetXInc, 0.0244+offsetYInc, Pose2D.deg2rad(45));
            obj.ir_array(2) = ProximitySensor(parent, 'IR', pose, ir_pose, 0.1, 0.8, Pose2D.deg2rad(6), 'simiam.robot.Jubileu.ir_distance_to_raw');
            
            ir_pose = Pose2D(0.0636+offsetXFron, 0.0, Pose2D.deg2rad(0));
            obj.ir_array(3) = ProximitySensor(parent, 'IR', pose, ir_pose, 0.1, 0.8, Pose2D.deg2rad(6), 'simiam.robot.Jubileu.ir_distance_to_raw');
            
            ir_pose = Pose2D(0.0461+offsetXInc,  -0.0396-offsetYInc, Pose2D.deg2rad(-45));
            obj.ir_array(4) = ProximitySensor(parent, 'IR', pose, ir_pose, 0.1, 0.8, Pose2D.deg2rad(6), 'simiam.robot.Jubileu.ir_distance_to_raw');
            
            ir_pose = Pose2D(-0.0690+offsetXLat, -0.0534, Pose2D.deg2rad(-90));
            obj.ir_array(5) = ProximitySensor(parent, 'IR', pose, ir_pose, 0.1, 0.8, Pose2D.deg2rad(6), 'simiam.robot.Jubileu.ir_distance_to_raw');
