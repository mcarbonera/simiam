...
%             obj.p = simiam.util.Plotter();
        ...
        function outputs = execute(obj, robot, state_estimate, inputs, dt)
            ...
            %u_ao = sum(u_i,2) - (vet_cont)*[cos(theta);sin(theta)];
            
            K_vet_cont = sensor_gains(3);
            u_ao = sum(u_i,2) + [cos(theta) -sin(theta); sin(theta) cos(theta)]*[-0.7*sqrt(2);0]*K_vet_cont;
            ...
            % 3. Blend 2 vectors
            alpha = 0.3;
            u_ao_gtg = alpha*u_gtg_n + (1-alpha)*u_ao_n;
