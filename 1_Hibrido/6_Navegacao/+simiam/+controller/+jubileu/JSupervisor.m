classdef JSupervisor < simiam.controller.Supervisor
...
% see the LICENSE file included with this software
    ...
    %% PROPERTIES
        ...
        v_max_w0            % Jubileu's max linear velocity when w=0
        w_max_v0            % Jubileu's max angular velocity when v=0
        v_min_w0
        w_min_v0
...
% Estratégia Blending com Hard-Switching
%Potenciamente converge para mínimo local em ambientes com obstáculos de formato côncavo.
%                if(obj.check_event('at_goal'))
%                    obj.waypoint_index = obj.waypoint_index + 1;
%                    if(obj.waypoint_index > size(obj.waypoints,1))
%                        obj.switch_to_state('stop');
%                    else
%                        obj.goal = obj.waypoints(obj.waypoint_index,:);
%                    end
...
% Estratégia - Seguidor de parede
                if obj.is_in_state('stop') && ~obj.check_event('at_goal')
                    obj.switch_to_state('ao_and_gtg');
                end
                
                if(obj.check_event('at_goal'))
                    obj.switch_to_state('stop');
                elseif((obj.is_in_state('go_to_goal') || obj.is_in_state('ao_and_gtg')) && (~obj.progress_made()))
                    %Slide left or Slide right??
                    if(obj.sliding_left())
                        %inputs.direction = 'left';
                        obj.fw_direction = 'left';
                        obj.switch_to_state('follow_wall');
                    elseif(obj.sliding_right())
                        %inputs.direction = 'right';
                        obj.fw_direction = 'right';
                        obj.switch_to_state('follow_wall');
                    end
                elseif(obj.is_in_state('follow_wall') && obj.progress_made())
                    if(strcmp(obj.fw_direction, 'left') && ~obj.sliding_left())
                        obj.switch_to_state('ao_and_gtg');
                    elseif(strcmp(obj.fw_direction, 'right') && ~obj.sliding_right())
                        obj.switch_to_state('ao_and_gtg');
                    end
                elseif(obj.is_in_state('ao_and_gtg') && obj.check_event('obstacle_cleared'))
                    obj.switch_to_state('go_to_goal');
                elseif(obj.is_in_state('go_to_goal') && obj.check_event('at_obstacle'))
                    obj.switch_to_state('ao_and_gtg');
                elseif(obj.is_in_state('avoid_obstacles') && ~obj.check_event('unsafe'))
                    obj.switch_to_state('ao_and_gtg');
...
%             fprintf('current_pose: (%0.3f,%0.3f,%0.3f)\n', x, y, theta);
        ...
        function rc = sliding_left(obj, state, robot)
            ...
            iop = obj.controllers{7}.is_obstacle_present;
            ...
            %fprintf('u_gtg: [%f,%f]\nu_ao: [%f,%f]\nu_fw: [%f,%f]\nsigma: [%f,%f]\n', u_gtg(1),u_gtg(2),u_ao(1),u_ao(2),u_fw(1),u_fw(2),sigma(1),sigma(2));
            %% END CODE BLOCK %%

            rc = false;
            if iop && sigma(1) > 0 && sigma(2) > 0
...
%                 fprintf('now sliding right\n');;
        ...
        function rc = progress_made(obj, state, robot)
            ...
            if(obj.goalChanged && rc)
                obj.goalChanged = false;
            elseif(obj.goalChanged)
                % Não fez progresso, mas vamos dizer que sim,
                % pois sabemos que é a inercia está provocando transições 
                % inconvenientes no autômato. Esperamos voltar a ter
                % progresso.
                rc = true
            end
            %% END CODE BLOCK %%
