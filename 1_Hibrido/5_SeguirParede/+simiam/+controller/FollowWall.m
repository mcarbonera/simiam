    ...
    methods
        ...
        function outputs = execute(obj, robot, state_estimate, inputs, dt)
            ...
            if(strcmp(inputs.direction,'right'))
                if(ir_distances(3) < obj.dist_sensor)
                    p_2 = ir_distances_wf(:,3);
                else
            ...
            %obj.filtro(:, obj.it_filtro) = u_fw_tp;
            %obj.it_filtro = obj.it_filtro + 1;
            %if(obj.it_filtro > obj.n_filtro)
            %    obj.it_filtro = 1;
            %end
            %u_fw_tp_filtrado = sum(obj.filtro,2)/obj.n_filtro;
            
            u_fw_p = (u_a - u_p) - dot((u_a - u_p), u_fw_tp)*u_fw_tp;
            %u_fw_p = (u_a - u_p) - dot((u_a - u_p), u_fw_tp_filtrado)*u_fw_tp_filtrado;
